...
if page == "🏠 Home":
    ...
    with col3:
        st.markdown(
            ...
            <p style="font-weight:600;">{CENTER_PHONE}</p>
    ...
    for idx, (g_date, d, m, y) in enumerate(upcoming):
        with ucols[idx % 3]:
            st.markdown(
                ...
                <b>{g_date.strftime('%A, %d %B %Y')}</b><br>
...
# ----------------------------------------------------------------------
    ...
    for h in hadiths:
        st.markdown(
            f"""<div class="hj-card">
            <p style="font-size:1.02rem;">"{h['text']}"</p>
